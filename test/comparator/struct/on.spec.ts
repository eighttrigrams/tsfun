...
import {sameset, is, equal, on, onBy, isnt, lt, gt} from '../../../src/comparator'
import { curry, flow } from '../../../src/composition'
...
describe('on', () => {
    ...
    it('on - with find and isArray', () =>
        ...
        expect(
            ...
            [{a: {b: [2, 1]}}, {a: {b: [2, 7]}}]
                .filter(onBy(sameset as any)(['a','b'])({a: {b: [1, 2]}}))
    ...
    it('intersectBy onBy equalTo - symmetric',() => {
        ...
        const $on = onBy(equal)
        const $intersect: any = curry(intersect, $on(['a', 'b']) as any)
